...
help:
	...
	@echo "                     The server side (socat relay, ufw rule, systemd unit, and"
	@echo "                     registering the resulting socks5h:// address via"
	@echo "                     PUT /v1/admin/proxy) is fully provisioned by deploy_f1.sh on"
	@echo "                     every deploy - nothing to set up by hand there."
	@echo "                     Running this from several devices (laptop, phone, a residential"
	@echo "                     proxy) needs a distinct SOCKS_PORT per device (1080, 1081, ...) -"
	@echo "                     list them all in the PROXY_TUNNEL_PORTS deploy variable (CI"
	@echo "                     vars/secrets), comma-separated."
